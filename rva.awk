BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["RVA_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["RVA_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["RVA_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["RVA_NOLINEWRAP"] != "" && ENVIRON["RVA_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
...
END {
	if (cmd == "ref" && outputted_records == 0) {
		print "Referencia no encontrada: " ref
	}
}
